...
$builder = [BuildProject]::new("MeristMomImImmune", $srcDirectory, $sdkPath, $gamePath)
...
if ($null -ne (Get-Command "X2ProjectGenerator.exe" -ErrorAction SilentlyContinue))
{
    ...
    &"X2ProjectGenerator.exe" "$srcDirectory\MeristMomImImmune" "--exclude-contents" "--verify-only"
